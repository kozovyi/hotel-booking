...
class RoomType(models.Model):
    ...
    room_type = models.CharField(
        max_length=20, choices=TypeChoice.choices, default=TypeChoice.STD
    )
    description = models.TextField()
...
class Room(models.Model):
    ...
    status = models.CharField(
        max_length=11, choices=StatusChoice.choices, default=StatusChoice.AVAILABLE
    )
    hotel = models.ForeignKey(
        Hotel, on_delete=models.CASCADE, related_name="hotel_rooms"
    )
    room_type = models.ForeignKey(
        RoomType, on_delete=models.PROTECT, related_name="type_room_rooms"
    )
